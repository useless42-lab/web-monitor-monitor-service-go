...
func GetLatestApiLog(apiId string, region string) RApiLogItem {
	var result RApiLogItem
	sqlStr := `select * from api_log where api_id=@apiId and region=@region order by id desc limit 1`
...
func AddApiLog(apiId string, status string, statusCode int, proto string, elapsed int64, responseData string, checkSuccess int, region string) {
	apiLog := ApiLogItem{
		ApiId:        apiId,
		Status:       status,
