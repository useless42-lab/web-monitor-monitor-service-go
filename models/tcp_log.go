...
func AddTcpLog(tcpId string, elapsed int, checkSuccess int, region string) {
	data := TcpLogItem{
		TcpId:        tcpId,
		Elapsed:      elapsed,
...
func GetLatestTcpLog(tcpId string, region string) RTcpLogItem {
	var result RTcpLogItem
	sqlStr := `select * from tcp_log where tcp_id=@tcpId and region=@region order by id desc limit 1`
