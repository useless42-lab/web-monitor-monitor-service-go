...
func GetLatestServerLog(serverId string, region string) RServerLogItem {
	var result RServerLogItem
	sqlStr := `select * from server_log where server_id=@serverId and region=@region order by id desc limit 1`
	DB.Raw(sqlStr, map[string]interface{}{
		"serverId": serverId,
		"region":   region,
	}).Scan(&result)
...
func CheckLatestServerLog(serverId string, startTime time.Time, endTime time.Time) bool {
	var result RServerLogItem
	sqlStr := `
	SELECT
	*
