...
func AddWebLog(webId string, status string, statusCode int, proto string, elapsed int64, responseData string, checkSuccess int, region string) {
	webLog := WebLogItem{
		WebId:        webId,
		Status:       status,
...
func GetLatestWebLog(webId string, region string) RWebLogItem {
	var result RWebLogItem
	sqlStr := `select * from web_log where web_id=@webId and region=@region order by id desc limit 1`
	DB.Raw(sqlStr, map[string]interface{}{
		"webId":  webId,
		"region": region,
	}).Scan(&result)
